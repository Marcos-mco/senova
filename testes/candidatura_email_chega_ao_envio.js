...
// Botão de mentira, mas de verdade: guarda textContent e dataset como o DOM guardaria.
function appComPill(vaga) {
  const btn = { textContent: '', dataset: {} };
  const s = carregarApp(EXTRAS, {
    document: { getElementById: id => (id === 'mv-doc-pill-resposta' ? btn : null) },
...
{
  // Achado ao escrever este teste, e é comportamento CERTO — fica travado para não se perder
  // numa unificação futura. Card nascido da caixa de entrada guarda em `emailDest` quem
  // escreveu; ali o pill é "Resposta por e-mail", porque houve um e-mail recebido. Só a
  // candidatura direta detectada na vaga (`canalDireto*`) autoriza o app a INICIAR o contato.
  const { s, btn } = appComPill({ id: 3, canal: 'Email', emailDest: 'recrutadora@empresa.com' });
  exec(s, 'mvSyncEnvioDireto()');
  t('card vindo de e-mail recebido continua em modo RESPOSTA, não vira envio proativo',
    btn.dataset.modoEnvio !== '1', btn.textContent);
  t('e o endereço de quem escreveu segue disponível para quem responder',
