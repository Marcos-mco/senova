...
*Atualizado: 31/ago/2026 — **Sessão 54 · Toda saída externa passa a ter uma porta só; a ideia dele vira um canal de vagas que não custa nada; um texto de fundo que eu escrevi faz ele duvidar do que o app guarda; e a estratégia vira do avesso — em vez de correr atrás de vaga, ser encontrado.**.*

***SESSÃO 54 — A PORTA ÚNICA, O CANAL DE GRAÇA, E A VIRADA PARA O INBOUND (29–31/ago).***

***Frente 1 — SSRF: toda saída externa passa por uma porta só (commit `3cdff6c`, Worker v7.61).*** *A dívida `(g)` da S52 fechada. A guarda existia e **protegia 1 de 2 rotas** — e mesmo na rota protegida **o redirect a anulava**: validava-se o endereço pedido, e o servidor remoto respondia "vá para cá", e para lá se ia sem perguntar. Agora existe `fetchExterno(alvo, opcoes)`, **única porta de saída**: `redirect:'manual'`, revalidação **a cada salto**, `MAX_SALTOS_EXTERNO=5`, `TETO_CORPO_EXTERNO=200000`.*

*__A lição da sessão está no review do meu próprio patch__: `localhost.` — com ponto final — **escapava da guarda**, porque o parser de URL normaliza o ponto e eu tinha suposto que ele não normalizava. **Medir o parser, não supor o que ele normaliza.** Achado por revisão depois de eu já considerar o trabalho pronto.*

***Frente 2 — Rüthen desligada, por palavra dele (commit `a02c505`).*** *A pergunta que a S53 deixou na fila foi respondida: presencial fora do Brasil sai da colheita. A frente da filha em Rüthen **fica desligada por decisão dele**, não por otimização de custo — que é exatamente a distinção que a S53 tinha se recusado a embaralhar.*

***Frente 3 — o canal de vagas que não custa nada, e a ideia é dele (commits `0b10969`, `56e934c`, Worker v7.62–v7.63).*** *__A pergunta dele:__ "que tal seguir a minha ideia de usar a IA para o que vem do email e varredura do google (sem custo) para outras vagas?" A fatia entregue: um campo no Perfil onde ele cola **endereços de feed dos alertas que ele mesmo criou**, e uma colheita que os lê **sem uma única chamada de IA no caminho** — os cards nascem com `score: null` e a nota só sai quando ele pede no card, como manda a regra do gesto da S53.*

*__Três decisões que valem para além desta feature.__ **(a)** Os cards de feed entram por `processarVagas`/`montarCard`, e não por um quinto gravador de `vagas_lead` — a família de defeito "N gravadores" custou caro demais na S47 e na S52. **(b)** `semFiltroCargo: true` para feed do usuário: o nosso vocabulário de cargo é o vocabulário de **uma** carreira, e repeneirar o que a própria pessoa buscou seria o app corrigindo o dono. **(c)** O recibo em KV guarda **`{linha, host}`, nunca o endereço** — um endereço de alerta é URL não-adivinhável com token embutido, e `colheita_feeds_status` é chave global: guardar o endereço ali entregaria a busca de um usuário aos outros. A pessoa reconhece a linha pela posição na lista dela.*

*__O freio que o crivo de viabilidade achou antes do commit:__ 10 endereços × 20 itens = 200 cards por rodada e **1.600 por dia** — 20× o volume que ele desligou em 23/ago por despejar demais. O teto por feed não segura isso porque é **por feed**. Entrou `NOVAS_POR_COLHEITA_FEED = 40`, teto **da rodada**; o que não entra não se perde, fica no feed e volta na colheita seguinte.*

*__E um achado que só apareceu porque eu rodei o feed de verdade__, em vez de ler o parser: `empresa` voltava **vazia**, porque cada gerador batiza a sua própria tag (`<job_listing:company>`, `<dc:creator>`, `<author><name>`). Sem o anunciante, o teto de 3 por anunciante não morde e uma agência que republica vinte vezes toma a colheita inteira. `_empresaDeFeed` busca pela **forma** do nome da tag, nunca pelo nome de um portal. Depois do conserto: **11 cards de anunciantes distintos** onde antes viriam 20 repetidos. **Testado em produção por ele: 11 vagas novas no Radar.***

***Frente 4 — o defeito que eu criei, e que não estava no código (commit `1209c75`).*** *Ele abriu a tela nova, colou o endereço que eu pedi, e escreveu: **"apagou os que estavam"**. Não tinha apagado nada — o campo sempre esteve vazio, e eu medi antes de responder: perfil íntegro, radar íntegro. Um minuto depois ele mesmo: *"talvez fossem exemplos de preenchimento. Não posso dizer."**

*__A causa era minha:__ o texto de fundo que eu escrevi eram **dois endereços plausíveis em cinza-claro**, e placeholder que parece valor salvo é indistinguível de dado que sumiu. **O dano não é estético: é uma pessoa desempregada deixando de confiar que o app preserva o que ela escreveu.** O `skill_ux_writing.md` já mandava o certo desde sempre — faltava **alguém medindo**. Nasceu `testes/texto_de_fundo.js` (5/5): texto de fundo com cara de valor tem de se anunciar como instrução ou exemplo. Ele **já pegou uma armadilha gêmea** no campo ao lado (`Amazon / Uber / Nubank`), que eu não tinha criado nesta sessão e nunca teria olhado.*

***MEDIDO E DELIBERADAMENTE NÃO ENTROU — a peneira geográfica.*** *Prometia cortar análise por localização. Medida: **0 de 42 análises ruins tinham problema de localização**. A maior linha viva de custo continua sendo `esteira_home`. Registrada com o número, como a peneira de cargo da S53.*

***Frente 5 — A VIRADA: em vez de correr atrás de vaga, ser encontrado.*** *__Ordem dele, verbatim:__ *"deixar o meu perfil o mais atraente possível para ficar nas preferências dos recrutadores… vamos utilizar mais energia em ser atrativo do que ficar atras de vagas. Isto também inclui não apenas o conteúdo, mas também a configuração, e tudo mais que possa intervir."** *É mudança de estratégia do produto inteiro, não um pedido de copy — **outbound → inbound**.*

*__Como o perfil dele foi lido:__ `WebFetch` no LinkedIn devolve **HTTP 999** (muro de autenticação) e **estar logado no navegador dele não muda nada**, porque a chamada sai de fora. O caminho que funcionou: **Perfil → Mais → Salvar como PDF**, que cai em `Downloads/Profile (1).pdf` e vem com tudo expandido. **Não há pdftoppm nem Python real nesta máquina** — o que funcionou foi `npx -y pdf2json@3` e um script de ordenação por (y,x) no scratchpad.*

*__O diagnóstico está publicado__ em **https://claude.ai/code/artifact/f9da9adb-f37e-44c2-ad7b-38d76651d54f** — headline e "Sobre" reescritos e prontos para copiar, cargo a cargo o que muda, a formação conferida contra os diplomas, as configurações em ordem de peso, e as três decisões com a minha recomendação.*

*__Os achados concretos, para não se perderem se o artifact sumir:__ **(1)** o cargo na Popper é literalmente **"Head"**, sem complemento — um ano e quatro meses invisíveis a qualquer busca; **(2)** duas experiências da Editel têm a cidade em **alfabeto cirílico** (`Куритиба, PR`), ou seja, cidade estrangeira para o filtro geográfico; **(3)** o mesmo negócio 2019–2024 aparece **duas vezes** (Marcenaria Fina / RIBEIRO FRANCO MF); **(4)** `Curiitba` no cargo atual; **(5)** headline usa **118 de 220 caracteres** e gasta parte com "CEO" e "CSO" enquanto deixa de fora "Diretor de Marketing", que foi o título dele por sete anos; **(6)** as datas de **Barcelona, FGV e FAAP divergem dos diplomas** — e vale o diploma, ordem dele; **(7)** os melhores números da carreira (R$ 40mi/900 escolas, R$ 20mi/25 agências, −73% mão de obra/+240% produção) estão enterrados nas descrições e ausentes do "Sobre".*

*__As três recomendações que ele pediu que eu desse__ (ele leu "decisão sua" e respondeu *"quero sua sugestão"*): **(a)** unificar 2019–2024 numa entrada só — empresa Ribeiro Franco MF, cargo Sócio-Fundador, função na primeira linha e marcenaria como ramo do negócio, dita com todas as letras; a Chippendale sai da formação acadêmica para "Licenças e certificados", **não por ser fraca, mas porque é munição contra o preconceito etário** e não pode diluir dois mestrados. **(b)** "MBA" só em português; Barcelona como *título propio* — e o argumento que fecha: **o mestrado oficial de Évora já responde por qualquer filtro de "Mestrado"**, então a honestidade sobre Barcelona não custa uma única busca. **(c)** manter telefone e e-mail públicos; o Hotmail é sinal datado, mas é a conta do Outlook que envia as candidaturas — ganho marginal, custo real, não mexer agora.*

***ESTADO AO FECHAR.*** *Último commit `1209c75`. Worker **v7.63** (`/health` confirma 7.63). Working tree limpo, `main` em dia com `origin/main`. **Suíte: 89 arquivos** — três nasceram nesta sessão: `saida_externa`, `feeds_do_usuario` (34/34), `texto_de_fundo` (5/5). **Catraca de universalidade: 153 pontos, sem subir.** `api.anthropic.com` no app: **zero**.*

***A PRÓXIMA SESSÃO É DEDICADA — decisão dele:*** ***atualizar o LinkedIn ponta a ponta***, um passo por vez, ele executando no navegador e eu conduzindo. **Começar pelos passos 1 e 2 da ordem de execução do artifact**: desligar "compartilhar atualizações do perfil" (para que as dezenas de edições não virem spam na rede dele) e corrigir os quatro erros. **Uma conferência que só ele pode fazer:** o Senova guarda o curso de Barcelona como *"Máster en Dirección de Marketing and Sales"* — espanhol e inglês na mesma linha; se a foto do diploma disser *"y Ventas"*, corrigir nos dois lugares.*

***DÍVIDA REGISTRADA, com sujeito.*** *__(a)__ **O LinkedIn Optimizer do app está quebrado e desalinhado** e não foi tocado: usa `CV_BASE` hardcoded em vez do Perfil; a lista "Experiência para reescrever" é fixa em **6** enquanto o Perfil tem **13**; usa `MODELOS.rapido`, reprovado com prova na S53; e há um bug real na linha final do prompt — `'liLang'==='EN'` compara o **literal** com "EN", então **pedido em inglês manda o modelo responder em espanhol**. __(b)__ Não-bloqueantes registrados pelo crivo na Frente 3: `higienizarRadar` pode ser monopolizada por cards de feed; `/api/vagas-lead` devolve blob de ~2,2 MB; `parsearFeed` materializa todos os blocos antes do `.slice(0, 20)`. __(c)__ Segue tudo o que a S53 deixou: régua v4 **não aplicada ao CV**; `donoSeguro` não alcança `executarVarredura`; `eua`×`us`; `index.html:7412-7417`; `"undefined"` como localização; `notas`/`descricao` trocados em `index.html:13331`; resto da fila anti-bloqueio da S52. __(d)__ **Sem teste dele ainda:** o card de Orçamento, e a vaga sem salário sob a régua v4.*

---

*Atualizado antes: 28/ago/2026 — **Sessão 53 · A fatura do cartão obriga o app a aprender a dizer não a si mesmo: teto de gasto vira trava dura, o preço aparece antes do gesto, o modelo barato é reprovado com prova, e a nota passa a admitir que há coisa que o anúncio simplesmente não informou.**.*

***SESSÃO 53 — O CUSTO VIRA ENGENHARIA, E A NOTA APRENDE A DIZER "NÃO SEI" (26–28/ago).***

***Frente 1 — o teto de R$ 200/mês vira número, não intenção (commits `872ca2a`, `3f0dc14`, Worker v7.46).*** *Ele abriu a fatura e o gasto de IA de agosto estava em R$ 285. O teto existia como intenção em documento; virou **trava dura no Worker**: estourado, a rota devolve **402 + `teto_atingido`** e nenhuma chamada sai. O mês do teto é **o da fatura de quem paga, não o do calendário** — corrigido em `3f0dc14`, porque um teto que vira no dia 1º enquanto a fatura vira no dia 15 não protege ninguém. Agosto fecha em R$ 285; **a partir de 01/set o teto é 200**.*

***Frente 2 — o preço antes do gesto, e a esteira que passou a pedir licença (commits `0c5957d`, `a239372`, Worker v7.47).*** *A primeira versão pôs o aviso de custo no botão "Buscar agora" — **um laço que ninguém aperta**. A medição mostrou onde o dinheiro sai de verdade: **`esteira_home` = 70% do gasto diário**, uma esteira que roda sozinha ao abrir a tela. Agora **a Home para de analisar por conta própria e pede licença**, e o preço aparece antes do gesto, não depois da fatura.*

***Frente 3 — o modelo 3× mais barato na triagem: REPROVADO, com prova (commits `3c68fdb`, `f19229b`, `1cfb8d0`, `ec1868c`, `3328969`, `9b239d5`, Worker v7.50–v7.53).*** *A hipótese era boa e o veredito foi não: sobre as mesmas vagas, o modelo barato **descarta 7 de 28 vagas boas** e lê "não informado" como zero. Mas o caminho até esse veredito é a lição maior — **três rodadas foram perdidas culpando o modelo por defeito nosso**: o `max_tokens` cortava a resposta pelo meio (v7.51), o parser jogava fora a análise inteira por uma frase depois do JSON (v7.52), e a falha não dizia por quê (v7.50). **Antes de culpar o de fora, medir a nossa camada.** Junto: **o modelo virou chave, não etiqueta** (v7.53) — o custo por modelo vinha com sujeito errado, e o aviso já estava na migração 005 sem que eu tivesse relido.*

***Frente 4 — o piso salarial estava escrito em três lugares (commits `732d2bf`, `fe1fb33`).*** *O Senova pedia 8 mil quando o piso dele é 12. E o piso **não era um número, eram dois**: 8 mil ficando em casa, 12 mil se tiver de mudar — o de ontem cortava Curitiba. Um valor de negócio com três gravadores é a mesma família de defeito que a S47 e a S52 já pagaram.*

***Frente 5 — o Perfil passa a mandar em onde se busca (commit `f4ba4e8`, Worker v7.57).*** *Ele desmarcou Espanha, Portugal, Alemanha e EUA no Perfil — e a varredura continuava varrendo tudo, porque as frentes eram lista nossa e o Perfil era enfeite. **91% da colheita eram países que ele já tinha desmarcado.** Agora o que ele desmarca para de ser varrido. __Uma frente ficou LIGADA de propósito e ele precisa saber:__ **Rüthen** (Alemanha) — ali o valor não é salário, é **proximidade da filha**, e por isso a frente nasceu com o filtro de cargo desligado. **Uma palavra dele fecha.***

***Frente 6 — duas economias que não podem mexer na nota (commits `5cda852`, `d14bd46`, Worker v7.56 e v7.58).*** *__(a)__ A análise em lote escrevia prosa que **só o card aberto lê** — saída enxuta na esteira. __(b)__ O **perfil complementar é o candidato, não a vaga**: é o mesmo texto em todas as vagas da esteira, e mesmo assim viajava na mensagem do usuário, que muda a cada vaga e por isso nunca cacheia. Virou **terceiro bloco de sistema cacheado**, na ordem rubrica → candidato → complementar (do mais estável ao menos, porque o cache é por prefixo). __A trava ética dessa mudança:__ o texto e a marcação foram movidos **caractere por caractere**, porque **economia que altera a nota não é economia, é outro produto** — e `testes/contexto_no_cache.js` guarda a redação, não só o mecanismo.*

***Frente 7 — o `/health` mentia havia dois releases (commit `e1ece59`).*** *Ele dizia **7.55** com a v7.57 no ar, porque a versão tinha **dois gravadores**: o cabeçalho e um literal na rota. Agora é uma constante e dois leitores, e `testes/versao_worker.js` cai se voltarem a discordar. Instrumento que mente sobre si mesmo é pior que instrumento nenhum.*

***Frente 8 — RÉGUA v4: o que o anúncio não informou para de virar nota inventada (commit `e065ebd`, Worker v7.59).*** *__O buraco, aberto desde a rubrica da S45:__ vaga que não declarava salário recebia nota em remuneração assim mesmo — o modelo improvisava entre 0 e 15 e **ninguém sabia qual improviso tinha sido**. Dois modelos diferentes chutavam diferente sobre a MESMA vaga, e a diferença chegava ao Kanban parecendo desacordo sobre o candidato. Não era: era desacordo sobre o que fazer com o que ninguém informou. **Zero afirma que a vaga é ruim naquilo; ausência afirma que não se sabe — e o app vinha trocando uma pela outra em silêncio.***

*__O conserto__ não é invenção nossa: fora do recrutamento, dado ausente é problema resolvido há décadas — tira-se do cálculo o que não se sabe e **registra-se que não se sabia**. A dimensão sem dado sai da soma **e do denominador**, o peso se redistribui, e a nota passa a viajar com a **cobertura**. O card diz **"nota sobre X% do que dava para saber"** quando isso for menor que 100 — duas notas iguais param de parecer iguais quando uma foi julgada inteira e a outra pela metade. A dimensão sem dado **aparece como "não informado"**, não some da linha: sumir faria a soma parecer não fechar e daria a impressão de que o Senova esqueceu de julgar aquilo.*

*__O acervo NÃO é repontuado__ — decisão dele, "o que está feito está feito". Nota v3 e nota v4 convivem no mesmo Kanban, e só podem conviver porque **a escala quase não se deslocou: medido sobre 28 vagas reais, +2,3 pontos em média, máximo 8**, e uma vaga mudou de lado em cada um dos cortes de 46, 55 e 70. Se esse deslocamento crescer, o acervo passa a mentir por comparação e a decisão de repontuar volta para a mesa — está escrito no código para que se possa detectar. `testes/regua_cobertura.js` (20/20) guarda o que sustenta tudo: **os tetos das cinco dimensões somam exatamente 100** — é o único motivo pelo qual a cobertura pode ser publicada com símbolo de porcentagem sem ser mentira.*

***O QUE FOI MEDIDO E DELIBERADAMENTE NÃO ENTROU — a peneira de cargo na colheita.*** *Uma peneira de títulos na varredura prometia **−19% de custo**. Medida sobre **279 vagas reais**, o número desmancha: **62 das 82 vagas presas no teto de 45 estão presas pela exigência de idioma**, que vive na descrição e é **invisível a uma peneira de título**; as 45 são lexicamente indistinguíveis das 70. E **~33 das 41 vagas que a peneira cortaria são a frente Rüthen** — ou seja, a "economia de 19%" é, na prática, **a decisão de parar de analisar onde a filha dele mora, vestida de otimização de custo**. Não entrou. Fica registrada com o número, não com a impressão.*

***ESTADO AO FECHAR.*** *Último commit `e065ebd`. Worker **v7.59** (`wrangler deploy` feito; `/health` confirma 7.59). Working tree limpo, push feito. **Suíte: 84 arquivos verdes** — três nasceram nesta sessão: `versao_worker`, `contexto_no_cache`, `regua_cobertura`. **Catraca de universalidade: 153 pontos, sem subir.** `api.anthropic.com` no app: **zero**. Backup `senova_v75_28ago2026_pre-regua-v4.html`.*

***PENDENTE DE TESTE DELE, e é o primeiro item da próxima sessão:*** *abrir uma vaga que **não declara salário** e ver a nota nova — o detalhamento deve mostrar "Remuneração não informado" e a linha terminar em "nota sobre 85% do que dava para saber". **O card de Orçamento também segue sem teste dele.***

***FILA AGUARDANDO PALAVRA DELE.*** ***(1)*** *Rüthen: fica ligada ou desliga junto?* ***(2)*** *O Perfil listar **frentes** (praça + raio + fontes + piso) em vez de países — é o desenho certo, e ele mesmo disse que "o design do perfil não estar totalmente reformulado atrapalha a busca".* ***(3)*** *O piso em euro das frentes internacionais (`18000`/ano) ainda é **suposição minha**, não número dele.* ***(4)*** *A pergunta dele que ficou sem fechar: **"precisamos de uma nota para a vaga e outra para o CV?"** — a proposta é que **não**: é a mesma régua vista de dois lados, e a cobertura da v4 é justamente o que torna isso honesto.*

***DÍVIDA REGISTRADA, com sujeito.*** *__(a)__ **A régua não dizia quanto vale um dado ausente** — fechado nesta sessão para a análise de vaga, **mas não para o CV**, que segue com a régua antiga. __(b)__ `donoSeguro` não alcança `executarVarredura`: **o cron não tem dono**, e por isso a varredura não lê o Perfil de ninguém no caminho automático. __(c)__ Descasamento de vocabulário **`eua` × `us`** nas frentes. __(d)__ Dívida de universalidade que a catraca não vê: `index.html:7412-7417` decide por `local.includes('brasil'|'espan'|'alemanha'|'eua')`. __(e)__ O literal `"undefined"` gravado como localização (`Itaú Unibanco · undefined`). __(f)__ `notas`/`descricao` trocados em `index.html:13331` para cards nascidos de alerta por e-mail. __(g)__ Toda a fila anti-bloqueio da S52 segue aberta, com a **brecha de SSRF em `/api/fetch-descricao`** ainda no topo — segurança antes de feature, sempre.*

---

*Atualizado antes: 25/ago/2026 — **Sessão 52 · A candidatura por e-mail volta a funcionar ponta a ponta depois que o LinkedIn removeu o JSON-LD; o Senova aprende a recuar quando um portal diz "pare"; e o CV que saía metade em inglês, metade em português tem a causa medida — a guarda de veracidade confundia formatação de número com fato inventado.**.*

***SESSÃO 52 — A CANDIDATURA VOLTA A SAIR, E O CV PARA DE SAIR PELA METADE (24–25/ago).***

***Frente 1 — o LinkedIn removeu o JSON-LD, e com ele foi embora a candidatura por e-mail (commit `e0191ec`, Worker v7.44).*** *A extensão lia a descrição da vaga por um bloco de dados estruturados que o LinkedIn simplesmente parou de publicar. Sem descrição não há e-mail do recrutador, e sem e-mail não há candidatura — o produto inteiro parava numa mudança de terceiro. **O guard estava verde com o produto quebrado**, e o motivo merece ficar escrito: ele testava a GRAVAÇÃO do campo, nunca o CONSUMO. Um teste que prova que o dado foi guardado não prova que alguém o lê.*

***Frente 2 — o endereço da candidatura tinha N leitores (commits `9fcab65`, `6f39adb`, `2ba2c63`).*** *A mesma família de defeito da S47 ("N gravadores"), do outro lado: `emailDest` era espelho, e cada tela lia de um lugar diferente — o card sabia o endereço e o botão de enviar não. Virou **ponto único de LEITURA, simétrico ao ponto único de gravação.** Junto: card criado à mão ganhou porta para enviar, e o e-mail do recrutador voltou a aparecer na Oportunidade salva.*

***Frente 3 — "Cuidado em não sermos bloqueados", ordem dele (commits `1f1e423`, `08b1ef4`, Worker v7.45).*** *Um portal que responde **429** ("pare") era respondido com mais seis requisições, porque o erro era engolido e a fila seguia. **O erro apagado virava repetição automática: 8.640 requisições/dia** contra portais que estavam pedindo trégua — o caminho mais curto para o Senova ser bloqueado em todos eles. Agora o 429 é lido, respeitado, e a fila recua. E o Worker **parou de se identificar como robô do Google** (`UA_SENOVA`): usar o nome de outro para passar por um portal é justamente o que a regra ética do projeto proíbe — e é o que faz um bloqueio virar bloqueio permanente. **Pendente de resposta dele:** se a requisição ao Bing News também deve passar à identificação própria.*

***Frente 4 — o erro grave: "Senova gerou um cv em inglês (que é o correto) mas com partes em português!".*** *A vaga da Jobgether (Head of Marketing). O Senova **decidiu certo** o idioma e entregou um PDF com o resumo e cinco experiências em inglês e **três experiências, a formação e os idiomas em português**, com a mesma confiança de um documento perfeito.*

*__A causa, medida pelo `senova-auditor` sobre o perfil real__ (não por leitura de código): a guarda de veracidade da S48 comparava **tokens de dígito** (`/\d+/g`), não quantidades. O inglês escreve milhar com vírgula, então "120 mil alunos" → "120,000 students" produzia os pedaços "120" e "000". Como "000" não existia no material original, a guarda concluía que a IA **inventara um número** e recusava a reescrita **daquela experiência inteira**, que caía de volta no texto em português, em silêncio, bloco a bloco. **Confundir FORMATAÇÃO com FATO NOVO — o único jeito de errar que produz meio documento.***

*__Por que a suíte estava verde com o produto quebrado__, e este é o achado que mais importa: `cv_bullets_por_vaga.js` só roda PT→PT, onde a grafia do número nunca muda entre línguas — este modo de falha era **estruturalmente impossível** ali. E `cv_idioma.js` traduzia bullets **sem número nenhum** (`['a','b','c']`). Pior: **três asserções afirmavam que o documento misto era comportamento correto** ("documento coerente, não vazio"). **Eram a documentação do defeito.** Foram reescritas. Nome novo para o anti-padrão, ao lado de "N gravadores" e "N leitores".*

*__O conserto__ (commit `a3eae1f`): a guarda compara **quantia**, não dígito — "120 mil" ≡ "120,000" ≡ "120.000"; "R$ 1,5 milhão" ≡ "1.5 million". Cada número rende as leituras possíveis e basta uma bater; **"1.500" é ambíguo entre línguas** (1500 em pt/de, 1.5 em en) e as duas valem — errar para o lado de reconhecer a mesma quantia, nunca para o de aceitar outra. **Número genuinamente novo continua barrado**: valor inflado, tempo de casa calculado e conversão de moeda reprovam. Do período passou a entrar **só o ano** — o mês ("2012-04") entrava como "4" e assinava qualquer "4" fabricado: a guarda estava frouxa exatamente onde parecia rigorosa. Toda recusa deixa **laudo com sujeito** (id, motivo, número acusado) — antes eram quatro `continue` mudos, e foi por isso que ninguém soube por que o documento saiu misto.*

*__E a trava que fecha a porta__: um CV em outra língua **não sai com bloco em português**. Se sobrar um, o PDF não é entregue; a mensagem nomeia os blocos e diz que o defeito é nosso. **Um CV meio traduzido é pior que um CV inteiro em português** — em português o recrutador entende que é outra língua; misto parece descuido do candidato. Ponto único em `_buildPDFExecDoc`, por onde passam os quatro caminhos de PDF. Junto, a **guarda irmã aplicada a nome**: o miolo do nome próprio de empresa e de instituição tem de sobreviver verbatim — "Universitat de Barcelona, Espanha" pode virar "…, Spain", nunca "University of Barcelona". É a lição da S49, "vale o que está nas fotos".*

***Frente 4b — "Já pedi para gerar um novo duas vezes" (commit `1c22828`).*** *A trava funcionou e recusou o documento — mas ele tentou duas vezes com o mesmo resultado. **Duas tentativas idênticas são sintoma de dois defeitos meus.** **(1)** A mensagem dizia QUAL bloco e não POR QUÊ — e sem o porquê, "gere de novo" é tentar a mesma coisa esperando outro resultado. O laudo já estava gravado no card (`atsPerfilTrad.recusados`) desde a manhã; **faltava o leitor**. **(2)** Sonda com os bullets reais da EADCon contra a guarda real: dez traduções plausíveis passam, e o furo apareceu num lugar só — **dígito colado em letra**. "B2B", "B2B2C", "K-12", "MP3": a guarda lia o "2" de B2B como fato inventado e recusava a experiência inteira. **Vocabulário corrente de marketing e de educação — ou seja, ela acusava a IA de mentir justamente onde a IA escrevia bem**, e no bloco (marketing educacional) onde isso era mais provável. "Top 3" continua barrado: aquilo é afirmação, não nome.*

***PENDENTE DE TESTE DELE, e é o primeiro item da próxima sessão:*** *gerar o CV da Jobgether outra vez. Se sair inteiro em inglês, era o B2B. Se a mensagem voltar, **agora ela diz o motivo** — e o print fecha o diagnóstico sem uma quarta tentativa. **Saída de emergência informada a ele:** o `.docx (ATS)` sai inteiro na língua certa, porque vem do texto que a IA escreveu, não da montagem que trava. **Também pendente de teste:** a verificação de link vivo/encerrado no card, depois do Worker v7.45.*

***DÍVIDA REGISTRADA NESTA SESSÃO, com número.*** *__(a)__ **`_expDaTelaParaCV` cunha `'exp_'+Math.random()`** para experiência sem id (`index.html:3126`): um bloco assim **nunca casa com a resposta da IA, em nenhuma vaga, para sempre** — mina achada pelo auditor, do mesmo tipo do bug desta sessão. __(b)__ **O `.docx` é montado do texto livre da IA e não passa pela guarda de números** — e construí-lo a partir da mesma estrutura do `_cvParaPDF` corta ~30% do custo do CV (os bullets são pagos duas vezes). Espinha dorsal: exige `senova-auditor` antes. __(c)__ **Repasse automático só dos ids recusados** custa R$ 0,062 contra R$ 0,37 de um "Refazer" inteiro — 6× mais barato, seguro porque o bloco já é casado por id; tem de nascer dentro de `montarPedidoCV`. __(d)__ **`origem:'cv'` não existe em `ORIGENS_CUSTO`**, então o volume de CV é hoje **imensurável**. __(e)__ Fila anti-bloqueio, aberta: "Verificar encerradas" dispara 188 requisições sem pausa; `portalBloqueou` não tem leitor no app e a tela ainda diz "faça login" num bloqueio; **`/api/fetch-descricao` é a única rota de saída externa sem `rateLimit` e sem `_hostProibido` (brecha de SSRF, exige `security-review`)**; cache de resposta de portal por id; pausa entre lotes de higiene; e os logs `[link-vivo/diag] TEMPORÁRIO` no ar desde 14/ago. __(f)__ **`_metaDoJsonLd` na extensão está mudo** desde a mudança do LinkedIn — o fix P5/S6 da S47 morreu junto.*

***ESTADO AO FECHAR.*** *Último commit `1c22828`. Worker **v7.45** (`npx wrangler deploy` feito). Extensão **2.80**. Working tree limpo. **Suíte: 73 arquivos verdes** (o teste novo `testes/cv_traducao_nao_sai_pela_metade.js` nasceu com 53 casos, usando os números reais da carreira dele). **Catraca de universalidade: 153 pontos, sem subir.** `api.anthropic.com` no app: **zero**. Backup `senova_v64_25ago2026_pre-cv-traducao.html`.*

***PRÓXIMOS PASSOS, nesta ordem.*** ***(1)*** *Fechar o CV da Jobgether — teste dele pendente.* ***(2)*** *A dívida (a) e (c) juntas: id estável para experiência da tela **e** repasse só dos ids recusados; é o mesmo caminho de código e é o que impede o defeito de voltar por outra porta.* ***(3)*** *A brecha de SSRF em `/api/fetch-descricao` — segurança antes de feature, sempre.* ***(4)*** ***Passo D do plano-chave*** (`.claude/plans/qual-o-plano-de-resilient-reddy.md`): D0 (teto de 3 tentativas + `user_id` no `custo_ia`), D1, D2, depois D3–D5. **É o pagamento da família C da catraca.** ***(5)*** *O resto da fila anti-bloqueio.* ***(6)*** *Passos E, F, G e o seletor de política de análise — que o parecer de viabilidade considera **mais importante para o MVP que o Passo G**.*

---

*Atualizado antes: 23/ago/2026 — **Sessão 51 · A busca automática de vagas é cancelada e fica só o e-mail, que é o canal que produz; e a regra "o Senova é para qualquer um, em qualquer lugar do planeta" deixa de ser documento e vira catraca executável, com a dívida medida em 153 pontos e teto que só pode cair.**.*

***SESSÃO 51 — O CORTE DO GASTO, E A REGRA QUE PRECISOU VIRAR MÁQUINA (23/ago).***

***Frente A1 — a varredura automática é cancelada (commit `d7b92e8`, Worker v7.42). TESTADA E APROVADA POR MARCOS.*** *Dez dias de `custo_ia` mostraram que **97,9% do gasto de IA é análise de vaga** (2.684 de 2.741 chamadas) e que a busca automática rendeu **2 processos e ZERO currículos enviados**, contra **211 processos e 6 currículos** dos alertas por e-mail. Ele foi além do que a equipe recomendou e decidiu: **"não é retirar apenas da Home, mas cancelar a varredura e só considerar o que vem do e-mail."** **A correção de fato que mudou a pergunta: o cron não gastava IA nenhuma** — ele só enchia a piscina no KV, e quem pagava era a esteira da Home (`_recalcLeadsReset`, até 60 rodadas × 5 = **300 análises por visita**). Por isso o corte precisou das duas metades: o cron diário saiu do `wrangler.toml` (o de 3 em 3 horas, que colhe e-mail, **fica**) e a esteira passou a recusar vaga vinda da piscina. **Nada foi apagado e há três caminhos de volta**, todos por ato dele: "Enviar para Processos", abrir a vaga pela extensão, e o "Buscar agora" — que agora **diz o custo antes do clique** (R$ 0,08/vaga, ~R$ 6,40 a rodada) e o rendimento medido. **A permissão viaja no card (`analisePedida`), não num modo global**, e a tela parou de mentir: vaga que ninguém vai avaliar diz "Sem análise · busca automática desligada", não "Avaliando…". Toda análise passou a **carimbar de qual esteira veio** (`esteira_home`, `card_aberto`, `extensao`) — sem isso, "cortar o radar" no mês que vem cortaria junto a análise do card que ele abre para se candidatar.*

***A regra que ele impôs no meio da sessão, e que vale acima de qualquer medição minha.*** *Diante do código que eu tinha acabado de escrever — `f==='adzuna'||f==='jobicy'` —: **"Vc não pode colocar portais específicos. Isto quem decide é o usuário!!!! O código fonte tem que estar limpo de qualquer informação já direcionada. Até mesmo para o outlook... uma pessoa na Alemanha pode escolher os portais que quer usar... vc está escrevendo um aplicativo para venda em qualquer lugar do planeta."** E depois: **"Temos que ter um Senova para qualquer um, utilizar em qq lugar do planeta. Não apenas eu."*** *O princípio que ficou no `CLAUDE.md`: **nome de serviço, de país ou de pessoa só pode viver em duas camadas — no adaptador que fala com aquele serviço, e no dado que o usuário configurou; nunca na camada que decide.** Falar com a Adzuna exige código da Adzuna (plugin, legítimo); o app perguntar "isto é Adzuna?" para decidir, não.*

***O modo de falha tem nome, e é recorrente: a medição de UM usuário virando lei para todos.*** *`DEFAULT_VAGAS` (S40), o dossiê servido sem credencial (S41), a semente do contexto complementar com a vida dele (S49), `PERFIL_MARCOS` concatenado em toda análise de todo usuário, e agora `_fonteVarredura`. **Nas cinco o dado estava certo — o erro foi escrever a conclusão como código em vez de como configuração do usuário.** Ele perguntou "como podemos evitar que isto não aconteça mais", e a resposta honesta é que já era regra escrita e falhou cinco vezes.*

***A catraca (commit `db6fdfc`).*** *`testes/senova_para_qualquer_um.js` mede a dívida em três famílias, fora de comentários: **A · a fonte decide** (campo de origem comparado a nome próprio) = **7**; **B · o nome do serviço decide** (portal ou provedor dentro de condição, ou farejado na URL) = **30**; **C · a identidade de uma pessoa está no código** = **116**. **Total: 153 pontos.** O teto de cada família **só pode cair**: passar dele reprova (dívida nova) e ficar abaixo também reprova, exigindo que o extrato seja corrigido — é assim que o número continua verdadeiro em vez de virar enfeite. **Um portal novo reprova sozinho**, porque o que se compara é o vocabulário de origem declarado do app, não uma lista de nomes proibidos que alguém teria que lembrar de atualizar (seria o mesmo erro outra vez). A trava tem **teste da própria mordida**: detector que para de achar é pior do que trava nenhuma. Limite declarado: `'linkedin'` ficou de fora da família B — 212 ocorrências, maioria legítima, precisa de triagem própria.*

***Achado desconfortável: o hook de pre-commit não existia.*** *`.git/hooks` só tinha os `.sample`. A suíte rodava porque eu lembrava de rodá-la — exatamente o tipo de trava que já falhou —, e a memória do projeto afirmava desde a S47 que havia hook. Agora está instalado e **versionado em `hooks/pre-commit`** (hooks se perdem em todo clone; reinstalar com `cp hooks/pre-commit .git/hooks/pre-commit`). Ele roda a suíte inteira mais o grep de `api.anthropic.com`, e já barrou/liberou o próprio commit da trava.*

***A ligação que ele mesmo fez, e que ordena o resto do plano: "inclusive por isso que estamos redesenhando totalmente o perfil".*** *Dos 153 pontos, **116 são a família C** — a vida dele dentro do código: os prompts que dizem "trabalhando exclusivamente para Marcos Franco", a saudação da tela, os três modelos de CV em PT/EN/ES, os placeholders "Curitiba, PR". **O Passo D não é melhoria de tela: é o pagamento de ~76% dessa dívida**, e agora dá para medir o progresso a cada commit em vez de achar que melhorou.*

***A ideia dele que vira o próximo passo — escolher o modo de análise, como se escolhe o modelo de IA.*** *"Para o MVP não podemos fazer com que o usuário escolha o padrão de busca... Analisar todas as vagas (varredura + e-mail); analisar apenas as que vêm do e-mail; analisar as selecionadas." **Isso não é conveniência: é o mecanismo que apaga o nome próprio do código.** Em vez de uma lista de portais na lógica, um **registro de fontes** onde cada fonte se declara e o usuário dá a política (analisar sempre / só quando eu pedir / não usar); a lógica passa a perguntar "que política o usuário deu a esta fonte?" e nunca "que fonte é esta?". Três correções de honestidade sobre a tela: **(1)** "analisar as selecionadas" não é um modo, é o piso que vale sempre — abrir um card analisa nos três, e o seletor precisa dizer isso, senão o modo mais barato parece "desligar a IA"; **(2)** cada opção diz o preço **medido**, e o do modo "só e-mail" só existe depois de ~7 dias rodando; **(3)** é por usuário, salvo no Perfil. **Sobre o Google**, pergunta dele: metade já está construída (o extrator de Alertas do Google no Worker, que chega pela caixa de e-mail, seja ela qual for) e é a única fonte que faz sentido vir ligada de fábrica — **mesmo assim como linha desligável do registro, não exceção no código.** E o alerta que precisa estar escrito: **trocar a fonte não economiza nada** — Adzuna, Jobicy e Google são chamadas HTTP grátis; o que custa é analisar cada vaga. Economiza **decidir antes de pagar**.*

***NÃO ENTREGUE NESTA SESSÃO, e é o primeiro item da próxima.*** *Marcos pediu para **zerar "Para Considerar"** e não consegue: **a seleção existe, mas alimenta uma única ação — "Enviar para Processos". Não há remoção em massa de uma seleção**, e o "Selecionar todas" só alcança as 15 linhas desenhadas. Régua dele: **"não invente nada temporário... comandos padrões de mercado. Gosto muito de como funciona o Outlook. Não exige curva de aprendizado."** O wireframe está desenhado e apresentado: barra de comando que aparece com a seleção ("12 selecionadas"), **Shift+clique** para intervalo, **Delete** para remover, **Esc** para limpar, e a **faixa do Gmail** ("todas as 15 desta página · selecionar as 132") para escapar da paginação. **Duas decisões já tomadas:** "Remover" em massa **arquiva** (recuperável, idêntico ao × de cada linha — não se inventa uma segunda semântica destrutiva), e "Apagar as N" continua sendo o apagamento definitivo, vermelho e à esquerda, como "Esvaziar Itens Excluídos". **Ctrl+A fica de fora de propósito** — ele pode estar tentando copiar um texto da tela. Zero nome próprio dentro; passa na catraca.*

***PRÓXIMOS PASSOS, nesta ordem.*** ***(1)*** ***Fix 1 — a seleção de mercado em "Para Considerar"***, *para ele conseguir zerar a lista. Está desenhado, falta codar.* ***(2)*** ***Fix 2 — o registro de fontes + o seletor de política de análise***, *que é a ideia dele e é o que apaga as famílias A e B da catraca (7 + 30 pontos), incluindo o Critério por país que hoje é lido do domínio do portal no link.* ***(3)*** ***A2 — parar de pagar duas vezes pela mesma vaga:*** *`_analiseFalhou` é memória de sessão, então toda vaga cuja análise falhou é reanalisada a cada recarga, para sempre. Passa a persistir com **teto de 3 tentativas**, e a redação é ordem dele: **"não! Dizer limite atingido. Ser honesto"** — "Limite de 3 tentativas atingido, o Senova parou de gastar análise nesta vaga", com a data da última e "analisar assim mesmo" à mão.* ***(4)*** ***Passo D — o esquema novo do Perfil*** `{plano_vida, projetos, fatos}`, *com migração na leitura e projeção de compatibilidade (o app aprende a ler o formato novo ANTES de o Worker gravá-lo — senão um perfil esquema-2 chega ao app esquema-1 como objeto válido e vazio, e o próximo "Salvar" grava o vazio por cima). **É o pagamento da família C.*** ***(5)*** *Passos E, F e G do plano da S50, inalterados.* ***(6)*** *Registrado, com a medição na mão: **dedup antes da IA** (potencialmente maior que os quatro cortes somados — `_vagaSimilarExistente` roda **depois** de a análise ter sido paga); **peneira determinística antes da IA** (`tituloRelevante` + knockout geográfico), com o descarte visível e o motivo na tela; **teto de gasto por usuário**; reconfirmar preço de token e câmbio; `max_tokens:1100` contra saída real de 562. **E a triagem do `'linkedin'`**, para fechar a família B da catraca.*

---

*Atualizado antes: 22/ago/2026 — **Sessão 50 · Retomada do plano combinado, rumo ao MVP. Três passos publicados e aprovados: os diplomas ganham caminho de saída (rede antes de tocar em dado), o app para de apagar a carreira salva por causa de uma resposta de rede, e o Perfil deixa de ser uma gaveta única global — passa a ser de quem o escreveu, com a régua de nota mínima voltando a ter uma casa só.**.*

***SESSÃO 50 — A FUNDAÇÃO DO PLANO DE VIDA COMEÇA A SER ASSENTADA (22/ago).***

***O pedido de Marcos, e a correção de rumo.*** *"Nós tínhamos combinado que começaríamos do zero o perfil... Sem remendos. Estamos indo para o MVP." A S49 tinha entregue um CV que finalmente é dele, mas pelo caminho antigo (`salvo || semente`) — ganhou-se dívida contra o plano `.claude/plans/woolly-seeking-matsumoto.md`. Ele mandou reunir a equipe antes de replanejar. **O `senova-auditor` achou doze colisões** entre o que o plano manda e o que a S49 construiu depois dele; o `senova-viabilidade` mediu o custo real e vetou quatro coisas. O plano da sessão (`.claude/plans/bruno-qual-o-plano-glowing-corbato.md`) é o combinado corrigido por esses dois pareceres. **A régua que ele deu e que vale acima de tudo: "cuidado para não prejudicar o Senova como está funcionando hj, pois preciso dele para candidaturas"** — gerar CV, analisar vaga e o Kanban funcionando ao fim de cada passo publicado; candidatura dele ganha de cronograma meu. **E a frase que ordena o produto inteiro: "emprego é apenas um módulo do Senova, que é um SaaS de Plano de vida."** Não é um app de vagas com camada de vida: é o contrário.*

***Decisões dele nesta sessão.*** *Fundação primeiro — Fixes 1→4 como um bloco só, a tela nova do Plano de Vida vem depois, sobre dado limpo. E **"Vale o Perfil: Brasil e remoto"**: as frentes europeias presenciais (Espanha, Portugal, Alemanha/Rüthen) saem da busca automática — a aplicar ao longo do caminho, com knockout geográfico que diz na tela por que a vaga foi descartada. Medido no radar vivo: 81 das 343 vagas (24%) vinham dessas frentes, e as 80 já pontuadas tiraram nota média 29 — o Senova pagava análise para descobrir que não serviam.*

***Passo A — a rede, antes de tocar em qualquer dado (commit `4a46609`).*** *Os oito diplomas guardados na S49 **não tinham caminho de saída**: a exportação varria só o armazenamento comum, e o cofre onde as fotos vivem (IndexedDB `senova_docs`), por desenho, não sobe para lugar nenhum. Limpar o navegador perdia tudo. "Faça backup antes de migrar" não cobria justamente o dado que só existe num lugar. Agora a exportação leva os diplomas, e **a cópia parou de se anunciar completa quando não era** — dizia "backup completo" enquanto deixava o cofre de fora.*

***Passo B — o apagador silencioso, fechado (commit `716c488`).*** *Achado do auditor, no caminho exato por onde a migração ia passar. `carregarPerfil` mandava para a tela o que voltasse do Worker **sem nunca perguntar se era mesmo um perfil**. Uma resposta de erro em JSON — Worker fora do ar, chave vencida, 401 — chegava como objeto sem os campos e: (1) esvaziava cada campo da tela, (2) `guardarExperienciasSalvas(undefined)` caía no `removeItem` e **apagava a carreira salva**, (3) e se ele clicasse em "Salvar perfil" para "resolver", o formulário em branco era gravado por cima do que estava no Worker — de onde não há volta. Nada disso avisava nada. **A regra que ficou: não saber não é o mesmo que ter deixado de existir. Quem só LÊ nunca apaga; apagar é ato de quem salva — e, sem ter conseguido ler, não se salva.***

***Passo C — Fix 1: o Perfil é de quem o escreveu (commit `8cc2f66`, Worker v7.41).*** *O Perfil inteiro morava numa chave única do KV (`perfil_usuario`): nome, telefone, carreira, formação, projeto de vida escrito à mão. **É de lá que a análise de vaga tira QUEM é o candidato.** O segundo usuário a salvar gravaria a vida dele por cima da do primeiro, e a análise sairia com a identidade trocada — no primeiro minuto do primeiro convidado. Agora é por dono, pelo mesmo `donoAtual` que os Processos já usam. **Três travas, todas com teste:** (1) a chave antiga não é apagada e continua espelhada enquanto for do mesmo dono — o caminho de volta é publicar a versão anterior, não restaurar backup; (2) o legado só é herdado por quem o escreveu (marcador `perfil_dono_legado`), então **usuário novo começa vazio, jamais com a vida de outra pessoa na tela** — a lição do `DEFAULT_VAGAS` (S40) e do vazamento de `/api/vagas-lead` (S41); (3) banco fora do ar não vira "o Senova esqueceu quem você é": `donoSeguro` devolve null e tudo opera pela chave antiga. **Junto morreu a duplicidade R9:** a régua de nota mínima por região tinha duas casas, e uma delas (`config_varredura`) é GLOBAL — a régua de uma pessoa valeria para todas. A casa passa a ser o Perfil; a config continua respondendo o campo, derivado, para não quebrar nenhum leitor (inclusive um app aberto há dias no navegador). **A migração rodou ANTES do deploy e foi conferida byte a byte** (30.235 bytes, sha256 idêntico), como manda a regra do "verificar antes de apagar". **`index.html` não foi tocado neste passo, de propósito:** zero risco para gerar CV, analisar vaga e Kanban.*

***Nota de método, que vale para toda asserção de teste.*** *Duas asserções antigas fixavam a **lista inteira** de argumentos de `analisarVaga` e caíam a cada argumento novo, sem nada ter quebrado. Passaram a guardar a **posição** do argumento, que é o que importa. Teste que quebra por motivo que não é regressão treina a gente a ignorá-lo.*

***O número que não mudou, e que decide o MVP.*** *R$ 0,0784 por análise, 2.568 análises em 9 dias → **R$ 398 a R$ 604/mês por usuário**, contra plano de R$ 29: distância de **69 vezes**. A cascata de cortes prevista vale ~60% e ainda assim não fecha. Três usuários de homologação hoje são ~R$ 1.200/mês na conta da Anthropic, e dois não pagam. **O que decide a margem do Radar não é o custo por análise: é quantas vagas ele analisa** — definido pelas frentes e pelo cron, não pelo uso. Antes do 2º usuário: bloco 1→4 fechado E teto de gasto por usuário.*

***Decisão de Marcos no fecho da sessão — "Para Considerar" sai.*** *Ele olhou a Home no ar ("61 para considerar") e decidiu: **"temos que ter limite sim, mas também podemos pensar outras formas de economizar... a gente analisa vagas mesmo abaixo do score; na Home, 'para considerar' podemos eliminar, pois agora no ar vi que não faz sentido."** É o depósito do refugo do radar (status `triagem`), criado para que nada sumisse em silêncio e auditado na S44 (910 cards, 0 CVs enviados a partir dele). **A ressalva que precisa estar escrita, para a próxima sessão não se enganar: tirar o depósito limpa a tela, mas não economiza um centavo** — a nota é o PRODUTO da análise, então quando a vaga chega ao "Para Considerar" o dinheiro já foi gasto (index.html:7193 e 11722 movem para lá **depois** de `/api/analisar-vaga` responder). **A economia de verdade está em descartar ANTES de pagar a IA** — o knockout geográfico determinístico do plano, e a triagem barata antes da cara. Os dois pedidos dele são, portanto, uma frente só: **decidir mais cedo, e mostrar na tela o que foi descartado e por quê** (veto 4 da equipe: nenhuma economia que esconda do usuário o que foi descartado — o `senova-viabilidade` também mediu que "3,5 análises por vaga distinta" é o número mais suspeito do parecer, e medir isso custa zero).*

***PRÓXIMOS PASSOS, nesta ordem (o bloco da fundação continua).*** ***(1)*** ***Passo D — Fix 2, o esquema novo `{plano_vida, projetos, fatos}`***, *com migração que aceita o formato antigo e converte sem destruir nada — **absorvendo o que a S49 criou depois do plano**: experiências, formação, contato, níveis de idioma, "Contexto do Perfil", e os campos que só existem chumbados no código (resumo profissional e localização). Carimbar `esquema:2` na 1ª gravação (R12).* ***(2)*** *Passo E — Fix 3: o Worker monta a análise a partir do Perfil real, e Perfil incompleto responde "complete seu plano de vida" em vez de analisar com a identidade de outra pessoa. **Correção do auditor: o CV não passa pelo Worker** — precisa do gêmeo no app, senão a Compatibilidade fica honesta e o PDF continua saindo com a carreira de fábrica. Entra aqui o teto de projeção em tokens (sem ele o prompt vai de ~1.400 para ~5.000 e custa +R$ 28/mês em silêncio).* ***(3)*** *Passo F — dar casa ao que o Fix 4 apagaria: a reconciliação com o diploma, a trava `_perfilEhDaSemente`, e o resumo e os idiomas do CV, que não têm campo na tela.* ***(4)*** *Passo G — Fixes 4 e 10: apagar `PERFIL_MARCOS`/`PROJETO_DE_VIDA`/`CV_BASE` do Worker e do app, incluindo o **autofill do Copiloto** (`__senovaCartaoCandidatura`, index.html ~13677), que ainda preenche formulário de candidatura real lendo o bloco fixo por regex.* ***(5)*** ***Frente de economia, decidida por ele e a tratar como uma coisa só:*** *aplicar Brasil e remoto **com knockout geográfico determinístico ANTES da IA**, tirar o "Para Considerar" da Home, e medir de onde vêm as 3,5 análises por vaga distinta. Ordem obrigatória: **medir primeiro, cortar depois** — e cada descarte diz na tela por quê, com a vaga acessível. Junto entra o teto de gasto por usuário, que ele confirmou querer ("temos que ter limite sim").* ***(6)*** *Fora desta rodada, registrado: tela nova do Plano de Vida e as quatro portas (Fixes 5-9); rotação de chave (`donoAtual`, R10 — hoje trocar a credencial cria usuário órfão); reconfirmar preço de token e câmbio; medir de onde vêm 3,5 análises por vaga distinta; entrevista da Sofia; remover o log `TEMPORÁRIO` de `/api/link-vivo`; S2/S3/S7 do backlog `metaConhecida`; `testes/cv_render.js` é citado em comentário e não existe.*

---

*Atualizado antes: 22/ago/2026 — **Sessão 49 · O Perfil passa a mandar no CV (experiências, formação e contato saem da tela, não do bloco fixo), os diplomas passam a ser guardados de verdade neste navegador, e a formação passa a dizer o que está escrito no papel. Nasce a reconciliação com o documento: quando o salvo diverge do diploma, o Senova mostra os dois lados e a pessoa decide — e quem edita por último manda.**.*
