...
*Atualizado: 21/jul/2026 — **Sessão 33 · CV: vazamento de análise no PDF fechado com portão único (setCV/setStatus + guard + pre-commit), diagramação final pelo Brand Book aprovada por Marcos e commitada**.*
...
**NÃO fechado (honesto):** o QA final do CV que Marcos pediu (skill/agente cobrindo veracidade + eficácia ATS + ortografia + adequação à vaga + design) ainda não foi construído — só existem testes parciais (`cv_pdf.js` testa a lógica do cabeçalho, `cv_estrutura.js` testa a estrutura de dados; nenhum dos dois vê o PDF renderizado). Também em aberto, como decisão de CONTEÚDO separada da diagramação: o CV com todas as 9 experiências marcadas `incluir_por_padrao` no perfil ocupa 2 páginas — o mockup original mostrava uma curadoria menor (7 + linha de "início de carreira"); Marcos ainda não decidiu se quer enxugar.
